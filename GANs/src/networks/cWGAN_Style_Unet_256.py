...
class Decoder(nn.Module):
    def __init__(self, z_dim, w_dim, in_channels, img_channels=1, skip_latent=True, tahn_act=False):
        ...
        self.genBlock_7 = GenBlock(in_channels=in_channels//32, out_channels=in_channels//64, kernel_size=3, stride=1, padding=1, w_dim=w_dim, double=double, last=True) #(256,256,256)
    ...
    def forward(self, noise, encoder_list):
        skip_1, skip_2, skip_3, skip_4, skip_5, skip_6, latent_tensor = encoder_list
...
class Encoder(nn.Module):
    def __init__(self, in_channels=1024, latent_dim=512, img_channels=2):
        ...
        self.block_1 = EncBlock(in_channels=img_channels, out_channels=in_channels//32, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x):
        out_1 = self.avg_pool(self.block_1(x))
