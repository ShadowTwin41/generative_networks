...
import nibabel as nib # TO remove
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        if use_label_cond_conv:
            print(f"USING CONDITION BLOCK")
            print(f"The condition channels from {label_cond_in_channels} -> {label_cond_in_channels*8} and shape reduced to half.")
            print("In case of poor tumor quality/poor tumor delineation, consider increasing the number of exit channels")
            self.label_condition_in = conv_nd(dims, label_cond_in_channels, label_cond_in_channels*8, 3, stride=(2,2,2), padding=(1,1,1))
        else:
            print(f"NOT USING CONDITION CONV. use_label_cond_conv: {use_label_cond_conv}")
    ...
    def to(self, *args, **kwargs):
        ...
        if isinstance(args[0], (list, tuple)) and len(args[0]) > 1:
            ...
            self.input_blocks.cuda()  # TODO .to(self.devices[0])
            self.time_embed.cuda()  # TODO .to(self.devices[0])
            self.middle_block.cuda()  # TODO .to(self.devices[0])  # maybe devices 0
            for k, b in enumerate(self.output_blocks):
                if k < self.decoder_device_thresh:
                    b.cuda()  # TODO .to(self.devices[0])
                else:  # after threshold
                    b.cuda()  # TODO .to(self.devices[1])
            self.out.cuda()  # TODO .to(self.devices[0])
    ...
    def forward(self, x, timesteps, label_condition, y=None):
        """
        Apply the model to an input batch.

        :param x: an [N x C x ...] Tensor of inputs.
        :label_condition: an [N x C x ...] Tensor of inputs. (original size)
        ...
        ###################################################################
        h = x
        if self.use_label_cond_conv:
            cond = self.label_condition_in(label_condition)
            h = th.cat((h, cond), 1)
        ...
        for k, module in enumerate(self.output_blocks):
            new_hs = hs.pop()
            if k == self.decoder_device_thresh:
                h = h.cuda()  # TODO .to(self.devices[1])
                emb = emb.cuda()  # TODO .to(self.devices[1])
            if k >= self.decoder_device_thresh:
                new_hs = new_hs.cuda()  # TODO .to(self.devices[1])
            ...
            h = module(h, emb)
        h = h.cuda()  # TODO .to(self.devices[0])
        return self.out(h)
