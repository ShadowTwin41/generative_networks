...
from  monai.transforms import RandAffined, RandFlipd, RandRotate90d, Compose
...
class GaussianDiffusion:
    ...
    def p_mean_variance(self, model, label_condition, x, t, mode, use_label_cond_conv, use_wavelet, clip_denoised=True, denoised_fn=None, model_kwargs=None):
        """
        Apply the model to get p(x_{t-1} | x_t), as well as a prediction of
        the initial x, x_0.
        :param model: the model, which takes a signal and a batch of timesteps
                      as input.
        :param label_condition: label condition
        ...
        if use_label_cond_conv:
            # The concat is done in the Unet
            x_in = x
        else:
            x_in = torch.cat((x, label_condition), dim=1)
        
        model_output = model(x_in, self._scale_timesteps(t), label_condition=label_condition, **model_kwargs)
        ...
        def process_xstart(x, use_wavelet):
            ...
            if clip_denoised:
                if use_wavelet:
                    ...
                    for model_output_idx in range(0, x.shape[1], 8):
                        x_idwt = idwt(model_output[:, 0+modal_idx*8, :, :, :].view(B, 1, H, W, D) * 3.,
                        ...
                        x_idwt_L.append(x_idwt)
                        modal_idx +=1 
                    x_idwt = torch.cat(x_idwt_L, dim=1)
        ...
        if self.model_mean_type == ModelMeanType.PREVIOUS_X:
            pred_xstart = process_xstart(
                self._predict_xstart_from_xprev(x_t=x, t=t, xprev=model_output), use_wavelet=use_wavelet,
            )
            model_mean = model_output
        elif self.model_mean_type in [ModelMeanType.START_X, ModelMeanType.EPSILON]:
            if self.model_mean_type == ModelMeanType.START_X:
                pred_xstart = process_xstart(model_output, use_wavelet=use_wavelet)
            else:
                pred_xstart = process_xstart(
                    self._predict_xstart_from_eps(x_t=x, t=t, eps=model_output), use_wavelet=use_wavelet
                )
    ...
    def p_sample_loop(
        ...
        for sample in self.p_sample_loop_progressive(
            ...
            final = sample
        return final["sample"], label_condition
    ...
    def p_sample(
        ...
        :param label_condition: condition label.
    ...
    def p_sample_loop_progressive(
        ...
        each timestep of diffusion. Takes label_condition as condition
        ...
        print(f"Number of steps for inference: {len(indices)}")
        if progress:
            # Lazy import so that we don't depend on tqdm.
            from tqdm.auto import tqdm
            indices = tqdm(indices)
        
        if mode=="tumour_inpainting":
            print("DOING MODE tumour_inpainting")
            # label_condition (shape (1,4,128,128,128)) -> No contrast, contrast, segmentattion and Healthy CT scan cropped to shape (1,1,128,128,128)
            healthy_ct_scan = label_condition[:, -1:, :, :, :] 
            segmentation_not_dilated = label_condition[:, 2:3, :, :, :]
            
            if label_condition.shape[1]==5:
                print("Blured mask")
                mask = label_condition[:, 3:4, :, :, :] # blured_mask
                label_condition = label_condition[:, 0:-2, :, :, :]  
            else:
                mask = segmentation_not_dilated
                label_condition = label_condition[:, 0:-1, :, :, :]  
                

        elif mode=="default_tumour_inpainting":
            print("DOING MODE tumour_inpainting")
            label_condition = label_condition[:, 0:-1, :, :, :] 
        ...
        for i in indices:
            ...
            if mode=="tumour_inpainting":
                # Takes the region to generate (tumour) from the predicted case and
                #   the known part of the healthy ct scan
                roi = img * mask 

                new_noise = th.randn_like(healthy_ct_scan).to(healthy_ct_scan.device) 
                noisy_healthy_ct_scan  = self.q_sample(healthy_ct_scan, t-1, noise=new_noise)  # Adds noise to the known portion of the CT scan for the next step
                not_roi = noisy_healthy_ct_scan * (1-mask)
    ...
    def training_losses(self, model,  x_start, t, mode, label_cond=None, segmentation=None, use_wavelet=None, model_kwargs=None, noise=None, tumour_weight=None):
        """
        Compute training losses for a single timestep.
        :param model: the model to evaluate loss on.
        :param x_start: the [N x C x ...] tensor of inputs - original image resolution.
        :label_cond: condition used for the model 
        :segmentation: in case we want the segmentation to be separated from the remaining label_cond
        :param t: a batch of timestep indices.
        :param model_kwargs: if not None, a dict of extra keyword arguments to
            pass to the model. This can be used for conditioning.
        :param noise: if specified, the specific Gaussian noise to try to remove.
        :param tumour_weight: weight for the tumour loss
        ...
        if mode == 'conv_before_concat':
            if use_wavelet:
                ...
                raise ValueError(f'Mode {mode=} not implemented when use_wavelet={use_wavelet}. use_wavelet needs to be True or mode needs to be "tumour_inpainting"')
        elif mode == 'concat_cond':
            if use_wavelet:
                # Wavelet transform the input image
                LLL, LLH, LHL, LHH, HLL, HLH, HHL, HHH = dwt(x_start)
                x_start_dwt = th.cat([LLL / 3., LLH, LHL, LHH, HLL, HLH, HHL, HHH], dim=1)
                ...
                # Add noise to case
                x_t = self.q_sample(x_start_dwt, t, noise=noise_dwt)  # Sample x_t
                # Concat condition to the noisy scan
                ...
                label_cond_down = resize(label_cond[0]).unsqueeze(0)
                x_t = torch.cat((x_t, label_cond_down), dim=1)
        elif mode == 'wavelet_cond':
            if use_wavelet:
                # Wavelet transform the input image
                LLL, LLH, LHL, LHH, HLL, HLH, HHL, HHH = dwt(x_start)
                x_start_dwt = th.cat([LLL / 3., LLH, LHL, LHH, HLL, HLH, HHL, HHH], dim=1)
                ...
                raise ValueError(f'Mode {mode=} not implemented when use_wavelet={use_wavelet}. use_wavelet needs to be True')
        elif mode == "default_tumour_inpainting":
            if use_wavelet:
                raise ValueError(f'Mode {mode=} not implemented when use_wavelet={use_wavelet}. use_wavelet needs to be False or mode needs to be "concat_cond" or "conv_before_concat"')
            # x_start is the input already
            if label_cond.shape[1]==4:
                raise Exception(
                    f'label_cond has shape {label_cond.shape[1]}. Probably the seg_dilated is being cocatenated as well.\n For mode default_tumour_inpainting, label_cond should contain no_contrast, contrast and seg')
        
            # noise is used to create the noisy scan 
            x_t = self.q_sample(x_start, t, noise=noise)  # Sample x_t
            x_t = torch.cat((x_t, label_cond), dim=1)

        elif mode == "tumour_inpainting":
            if use_wavelet:
                raise ValueError(f'Mode {mode=} not implemented when use_wavelet={use_wavelet}. use_wavelet needs to be False or mode needs to be "concat_cond" or "conv_before_concat"')
            # x_start is the input already
            if label_cond.shape[1]==4:
                # label_cond contains no_contrast, contrast and seg, seg_dilated
                # The noise is added only to the region of the tumour and surrounding tissue. 
                # The remaining is known (without noise)
                segmentation_dilated = label_cond[:, 3:4, :, :, :]
                noise = noise * segmentation_dilated 
            ...
            raise ValueError(f'Invalid mode {mode=}, needs to be "conv_before_concat", "concat_cond" or "tumour_inpainting"')
        
        model_output = model(x_t, self._scale_timesteps(t), label_condition=label_cond, **model_kwargs)  # Model outputs denoised wavelet subbands

        if use_wavelet:
            # Inverse wavelet transform the model output
            B, _, H, W, D = model_output.size()
            for modal_idx ,_ in enumerate(x_start):
                model_output_idwt = idwt(model_output[:, 0+modal_idx*8, :, :, :].view(B, 1, H, W, D) * 3.,
                                        model_output[:, 1+modal_idx*8, :, :, :].view(B, 1, H, W, D),
                                        model_output[:, 2+modal_idx*8, :, :, :].view(B, 1, H, W, D),
                                        model_output[:, 3+modal_idx*8, :, :, :].view(B, 1, H, W, D),
                                        model_output[:, 4+modal_idx*8, :, :, :].view(B, 1, H, W, D),
            ...
            if tumour_weight != 0:
                # Computes the MSE only of the tumour voxels (the tumour voxels are represented in label_cond as 1)
                real_tumour = label_cond[:, -1, :, :, :] * x_start[:, 0, :, :, :]
                fake_tumour = label_cond[:, -1, :, :, :] * model_output_idwt[:, 0, :, :, :] 
                tumour_loss = th.mean((real_tumour - fake_tumour) ** 2)
                terms = {"mse": th.mean(mean_flat((x_start_dwt - model_output_idwt) ** 2), dim=0), "mse_tumour": tumour_loss}
            else:
                if segmentation!=None:
                    terms = {"mse": th.mean(mean_flat(((x_start[segmentation == 0]) - (model_output_idwt[segmentation == 0])) ** 2), dim=0)}     
                else:
                    terms = {"mse": th.mean(mean_flat((x_start - model_output_idwt) ** 2), dim=0)}
        else:
            model_output_idwt = model_output
            if tumour_weight != 0:
                real_tumour = label_cond[:, -1, :, :, :] * x_start[:, 0, :, :, :] # The last channel of the condition is always the segmentation
                fake_tumour = label_cond[:, -1, :, :, :] * model_output[:, 0, :, :, :] 
                tumour_loss = th.mean((real_tumour - fake_tumour) ** 2)
                terms = {"mse": th.mean(mean_flat((x_start - model_output) ** 2), dim=0), "mse_tumour": tumour_loss}
            else:
                terms = {"mse": th.mean(mean_flat((x_start - model_output) ** 2), dim=0)}

        return terms, model_output, model_output_idwt, label_cond
