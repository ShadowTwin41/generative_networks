...
"""
From: https://github.com/cyclomon/3dbraingen/blob/master/pytorch_ssim/__init__.py
"""
...
def msssim_3d(img1, img2, window_size=11, size_average=True, val_range=None, normalize=False):
    ...
    #mcs = np.asarray(mcs)
    mcs=torch.FloatTensor(mcs)
    mssim=torch.FloatTensor(mssim)
    # Normalize (to avoid NaNs during training unstable models, not compliant with original definition)
    if normalize:
        mssim = (mssim + 1) / 2
        mcs = (mcs + 1) / 2
        
    pow1 = mcs.to(device='cuda') ** weights.to(device='cuda')
    pow2 = mssim.to(device='cuda') ** weights.to(device='cuda')
