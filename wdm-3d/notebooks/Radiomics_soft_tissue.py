...
def create_binary_seg(labelsTr, nnunet_id, binary_labelsTr):
    save_folder = binary_labelsTr
    ...
    for file_name in tqdm(listdir(labelsTr)):
        ...
        output_path = join(binary_labelsTr, file_name)
...
binary_labelsTr = join(datasets_save_folder, nnunet_id, 'soft_tissue_labelsTr')

# Run the Binary mask creator
create_binary_seg(labelsTr, nnunet_id, binary_labelsTr)
...
output_csv = join(output_folder, "soft_tissue_radiomics.csv")
